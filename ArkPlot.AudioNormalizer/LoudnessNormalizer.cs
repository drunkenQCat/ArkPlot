...
public class LoudnessNormalizer
{
    ...
    /// 为 null 时按优先级自动查找。
    /// </param>
    public LoudnessNormalizer(string? ffmpegPath = null)
    {
        var path = ffmpegPath ?? FfmpegResolver.FindFfmpeg()
    ...
    {
        var path = FfmpegResolver.FindFfmpeg();
        if (path != null)
            return new LoudnessNormalizer(path);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return LoudnessMeasurement.Parse(sb.ToString());
    }
